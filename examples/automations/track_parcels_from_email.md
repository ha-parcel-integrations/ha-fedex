# Automatic parcel tracking from e-mail (IMAP → FedEx)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the FedEx tracking code, and register it with `fedex.track_parcel` — fully automatic, no extra custom component required.

FedEx is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `fedex.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ fedex.track_parcel
                                     │
...
- This integration, with the `fedex.track_parcel` action available (field `tracking_code`).
...
The regex matches a standalone FedEx tracking number of 12 to 20 digits:

```
(?<![0-9A-Za-z])[0-9]{12,20}(?![0-9A-Za-z])
```

FedEx uses multiple tracking-number formats; this conservative example covers the common numeric 12–20 digit form. The integration itself accepts any non-empty code, while the AI fallback can handle formats not matched by this local regex.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@fedexshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `fedex.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
