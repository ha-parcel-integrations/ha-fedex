"""Calendar platform for the FedEx parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the FedEx deliveries calendar from a config entry."""
    async_add_entities([FedExDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class FedExDeliveriesCalendar(CoordinatorEntity[FedExCoordinator], CalendarEntity):
    """A read-only calendar of expected FedEx deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "FedEx parcel")
            description_parts = [
