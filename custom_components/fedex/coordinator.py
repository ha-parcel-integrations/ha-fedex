"""Coordinator for the FedEx parcel tracker integration.
...
class FedExCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for result in results:
            if isinstance(result, FedExAuthError):
                raise ConfigEntryAuthFailed("FedEx API key rejected") from result
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(result, (FedExApiError, aiohttp.ClientError)):
                    raise result
                errors += 1
                if isinstance(result, FedExApiError) and result.status_code == 429:
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("FedEx fetch failed for %s: %s", code, result)
        ...
        if saw_429:
            ...
            raise UpdateFailed("FedEx rate-limited (429)", retry_after=retry_after)
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("FedEx unreachable for all tracked parcels")
        ...
        self._delivered_codes = {
            code for code, parcel in entries if parcel["delivered"]
        }
