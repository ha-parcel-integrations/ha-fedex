"""Sensor platform for the FedEx parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: FedExConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up FedEx sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        FedExIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            FedExParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(FedExNextDeliverySensor(coordinator, entry))
    entities.append(FedExAwaitingPickupSensor(coordinator, entry))
    entities.append(FedExDeliveredParcelsSensor(coordinator, entry))
    entities.append(FedExLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class FedExIncomingParcelsSensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                FedExParcelSensor(self.coordinator, self._entry, barcode)
...
class FedExParcelSensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked FedEx parcel."""
    ...
    def __init__(
        self, coordinator: FedExCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class FedExNextDeliverySensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    ...
    def __init__(self, coordinator: FedExCoordinator, entry: ConfigEntry) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class FedExAwaitingPickupSensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    """Tracked parcels ready for collection at a FedEx pickup point."""
    ...
    def __init__(self, coordinator: FedExCoordinator, entry: ConfigEntry) -> None:
        """Initialise the pickup summary sensor."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return the number of parcels awaiting collection."""
        return len(self._parcels())

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the privacy-filtered canonical parcel summaries."""
        return {"parcels": self._parcels()}


class FedExDeliveredParcelsSensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    """Recently delivered tracked FedEx parcels."""
...
class FedExLastUpdateSensor(CoordinatorEntity[FedExCoordinator], SensorEntity):
    """Diagnostic sensor reporting when FedEx was last polled successfully."""
