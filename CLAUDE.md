...
Home Assistant custom integration for **FedEx** parcel tracking.
...
private `carrier-research/fedex/api/` and are **never** copied here.
...
- FedEx's real `HP` scan event reaches `ParcelStatus.AT_PICKUP_POINT`; the
  incoming `awaiting_pickup` sensor is therefore required. Use the generic
  term "pickup point"; no location is exposed until a privacy-safe field is
  confirmed in a production payload.

## Carrier-specific notes

FedEx is code-based and needs an official developer API key. A rejected OAuth credential raises
`ConfigEntryAuthFailed` for the entire coordinator, while ordinary per-package
errors retain cached data. Access tokens are memory-only; names, addresses,
references, signatures and coordinates stay out of normalised data and diagnostics.
The private wire reference is `carrier-research/fedex/api/`.
...
validated against the carrier's **official** API at setup, credentials in
`entry.data`, and a reauth flow for when the key is rotated or revoked outside
Home Assistant (`FedExAuthError` from any per-parcel fetch raises
...
refresh. `fedex/coordinator.py` is the canonical implementation
...
```
python -m pytest tests/ --cov=custom_components.fedex
```
