"""Tests for FedEx setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    incoming = hass.states.get("sensor.fedex_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.fedex.api.FedExApiClient.async_get_parcel",
        new=AsyncMock(side_effect=FedExApiError("FedEx unreachable")),
    ):
...
async def test_setup_starts_reauth_when_key_is_rejected(hass):
    """A key rotated or revoked outside HA must prompt reauth, not just retry."""
    entry = _entry([ACTIVE_CODE])
    entry.add_to_hass(hass)

    with patch(
        "custom_components.fedex.api.FedExApiClient.async_get_parcel",
        new=AsyncMock(side_effect=FedExAuthError("HTTP 401")),
    ):
        assert not await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.fedex.api.FedExApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
