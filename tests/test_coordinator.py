"""Tests for the FedEx coordinator: fetching, caching and events.
...
from custom_components.fedex.coordinator import FedExCoordinator
from custom_components.fedex.parcels import to_iso_timestamp
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = FedExApiError("HTTP 500")
...
async def test_update_raises_auth_failed_when_key_is_rejected(hass):
    """A rejected key is the whole poll's problem, not this one parcel's."""
    entry = _entry_with([{CONF_TRACKING_CODE: DELIVERED_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = FedExAuthError("HTTP 401")
    coordinator = FedExCoordinator(hass, client, entry)
...
async def test_auth_failure_on_one_parcel_still_raises_auth_failed(hass):
    """One tracked parcel rejected for auth is enough -- the key is shared."""
    entry = _entry_with(
        [{CONF_TRACKING_CODE: ACTIVE_CODE}, {CONF_TRACKING_CODE: DELIVERED_CODE}]
    )
    entry.add_to_hass(hass)
    ...
    client.async_get_parcel.side_effect = lambda code: (
        active_sample(code)
        if code == ACTIVE_CODE
        else (_ for _ in ()).throw(FedExAuthError("HTTP 401"))
    )
    coordinator = FedExCoordinator(hass, client, entry)
...
async def test_delivered_code_skipped_from_fetch(hass):
    ...
    client = AsyncMock()
    client.async_get_parcel.side_effect = lambda code: (
        active_sample(code) if code == ACTIVE_CODE else delivered_sample()
    )
    coordinator = FedExCoordinator(hass, client, entry)
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    moved["estimatedDeliveryTimeWindow"] = {
        "window": {
            "begins": "2026-04-29T16:00:00-05:00",
            "ends": "2026-04-29T18:00:00-05:00",
        }
    }
    ...
    assert len(events) == 1
    assert events[0].data["old_planned_from"] == to_iso_timestamp(
        "2026-04-29T13:00:00-05:00"
    )
    assert events[0].data["new_planned_from"] == to_iso_timestamp(
        "2026-04-29T16:00:00-05:00"
    )
