"""Tests for FedEx sensor property logic."""

from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.fedex.const import ParcelStatus
from custom_components.fedex.sensor import (
    FedExAwaitingPickupSensor,
    FedExDeliveredParcelsSensor,
    FedExIncomingParcelsSensor,
    FedExLastUpdateSensor,
    FedExNextDeliverySensor,
    FedExParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = FedExIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = FedExParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = FedExParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = FedExNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = FedExNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = FedExDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
    assert sensor.extra_state_attributes["parcels"][0]["barcode"] == "D"


def test_awaiting_pickup_sensor_lists_only_ready_parcels():
    coordinator = _coordinator(
        [
            _parcel("P", status=ParcelStatus.AT_PICKUP_POINT, pickup=True),
            _parcel("T", status=ParcelStatus.IN_TRANSIT),
        ]
    )
    sensor = FedExAwaitingPickupSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = FedExLastUpdateSensor(coordinator, _entry())
